...
namespace AFJK.Rapier.Samples
{
    ...
    /// (<see cref="RapierWorldBehaviour"/>, <see cref="RapierRigidbody"/>, the collider /
    ...
    public sealed class RapierComponentDemosSample : MonoBehaviour
    {
        ...
        // Platform demo state.
        private RapierRigidbody platformBody;
        private float platformPhase;

        // Character controller demo state.
        private RapierCharacterControllerBehaviour characterController;

        // PID controller demo state.
        private RapierRigidbody pidBody;
        private RapierPidController pidController;
        ...
        private void FixedUpdate()
        {
            ...
            // RapierWorldBehaviour.Step() runs the simulation and syncs every registered body's
        ...
        // Demonstrates RapierWorldBehaviour.RebuildWorld: assigns deterministic keys to every
        ...
        private void AssignDeterministicKeys()
        {
            ...
            var bodyComponents = worldGo.GetComponentsInChildren<RapierRigidbody>(true);
            ...
            var colliderComponents = worldGo.GetComponentsInChildren<RapierCollider>(true);
            ...
            var jointComponents = worldGo.GetComponentsInChildren<RapierJoint>(true);
        ...
        private void CreateCollisionGroupsGround()
        {
            ...
            var body = go.AddComponent<RapierRigidbody>();
        ...
        private void CreatePrismaticJointChain(Vector3 origin, int count)
        {
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                var joint = AddJoint<RapierPrismaticJoint>(parent, child, Vector3.zero, new Vector3(0f, 0f, -shift));
                joint.Axis = axis;
        ...
        private void CreateRevoluteJointChain(Vector3 origin, int count)
        {
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                var links = new RapierRigidbody[4];
        ...
        private void CreateRevolute(RapierRigidbody a, RapierRigidbody b, Vector3 anchor1, Vector3 anchor2, Vector3 axis)
        {
            var joint = AddJoint<RapierRevoluteJoint>(a, b, anchor1, anchor2);
            joint.Axis = axis;
            joint.gameObject.SetActive(true);
        }
        ...
        private void CreateFixedJointGrid(Vector3 origin, int count)
        {
            const float radius = 0.4f;
            const float shift = 1.0f;
            var parents = new List<RapierRigidbody>(count * count);
            ...
            for (var k = 0; k < count; k++)
            {
                for (var i = 0; i < count; i++)
                {
                    ...
                    if (i > 0)
                    {
                        AddJoint<RapierFixedJoint>(parents[parents.Count - 1], child, Vector3.zero, new Vector3(0f, 0f, -shift)).gameObject.SetActive(true);
                    }

                    if (k > 0)
                    {
                        AddJoint<RapierFixedJoint>(parents[parents.Count - count], child, Vector3.zero, new Vector3(-shift, 0f, 0f)).gameObject.SetActive(true);
                    }
        ...
        private void CreateSphericalJointGrid(int count)
        {
            ...
            for (var k = 0; k < count; k++)
            {
                for (var i = 0; i < count; i++)
                {
                    ...
                    if (i > 0)
                    {
                        AddJoint<RapierSphericalJoint>(parents[parents.Count - 1], child, Vector3.zero, new Vector3(0f, 0f, -shift)).gameObject.SetActive(true);
                    }

                    if (k > 0)
                    {
                        AddJoint<RapierSphericalJoint>(parents[parents.Count - count], child, Vector3.zero, new Vector3(-shift, 0f, 0f)).gameObject.SetActive(true);
                    }
        ...
        private void BuildCharacterController()
        {
            ...
            characterController = characterBody.gameObject.AddComponent<RapierCharacterControllerBehaviour>();
        ...
        private void BuildPidController()
        {
            ...
            pidController = pidBody.gameObject.AddComponent<RapierPidController>();
        ...
        private RapierRigidbody CreateStackBody(string id, int shapeKind, Vector3 position, float radius)
        {
            switch (shapeKind)
            {
                case 0:
        ...
        private RapierRigidbody CreateSphere(
            string id,
            RapierRigidBodyType bodyType,
            Vector3 position,
            float radius,
        ...
        private RapierRigidbody CreateCapsule(
            string id,
            RapierRigidBodyType bodyType,
            Vector3 position,
            float halfHeight,
        ...
        private RapierRigidbody CreateConvexCylinderBody(string id, Vector3 position, float radius, Color color, Vector3 linearVelocity, float halfHeight = -1f)
        {
        ...
        private RapierRigidbody CreateConvexConeBody(string id, Vector3 position, float radius, Color color, Vector3 linearVelocity)
        {
            const int segments = 16;
        ...
        private RapierRigidbody CreateConvexBody(string id, Vector3 position, Vector3[] points, Color color, float visualRadius)
        {
            var go = GameObject.CreatePrimitive(PrimitiveType.Sphere);
        ...
        private RapierRigidbody CreateConvexBodyWithMesh(string id, Vector3 position, Vector3[] hullPoints, Vector3[] vertices, int[] indices, Color color, Vector3 linearVelocity)
        {
        ...
        private RapierRigidbody CreateCompoundBody(string id, Vector3 position, float radius)
        {
            var go = NewBodyObject(id, position, Quaternion.identity);
        ...
        private RapierRigidbody CreateTrimeshBody(string id, RapierRigidBodyType bodyType, Vector3[] vertices, int[] indices, Color color)
        {
        ...
        private void CreateWorld(Vector3 gravity)
        {
            ...
            worldComponent = worldGo.AddComponent<RapierWorldBehaviour>();
        ...
        private T AddJoint<T>(RapierRigidbody body1, RapierRigidbody body2, Vector3 anchor1, Vector3 anchor2)
            where T : RapierJoint
        {
        ...
        private static T GetCollider<T>(RapierRigidbody body) where T : RapierCollider
        {
